...
def normalize_spot_slots(slots):
    """`[start, end, price]` timestamp triples. Drops junk, sorts by start."""
    out = []
...
def current_spot_price(slots, now_ts):
    last = None
...
def spot_raw_from_slots(clock, slots):
    """Chart attribute: `[{start, end, value}, ...]` with ISO times."""
    out = []
    for start, end, price in slots or []:
        start_iso = _iso(clock, start)
        end_iso = _iso(clock, end)
        if start_iso is None or end_iso is None:
            continue
        out.append({"start": start_iso, "end": end_iso, "value": price})
    return out


def slots_from_state_history(clock, history_states, slot_seconds=SLOT_SECONDS, period_end_ts=None):
    """Rebuild price slots from recorder rows of the Nordpool state."""
    parsed = []
    for st in history_states or []:
        if isinstance(st, dict):
            state = st.get("state")
            last_changed = st.get("last_changed") or st.get("last_updated")
        else:
            state = getattr(st, "state", None)
            last_changed = getattr(st, "last_changed", None) or getattr(
                st, "last_updated", None
            )
        price = _to_price(state)
        ts = _to_ts(clock, last_changed)
        if price is None or ts is None:
            continue
        parsed.append((ts, price))
    parsed.sort()
    try:
        step = float(slot_seconds)
    except (TypeError, ValueError):
        step = float(SLOT_SECONDS)
    if step <= 0:
        step = float(SLOT_SECONDS)
    slots = []
    for i, (start, price) in enumerate(parsed):
        if i + 1 < len(parsed):
            end = parsed[i + 1][0]
        elif period_end_ts is not None and period_end_ts > start:
            end = float(period_end_ts)
        else:
            end = start + step
        if end <= start:
            continue
        t = start
        while t + step < end - 1:
            slots.append([t, t + step, price])
            t += step
        slots.append([t, end, price])
    return slots


def merge_spot_series(clock, stored, live, now_dt, keep_days=SPOT_KEEP_DAYS):
    """Realized past from storage, current+future from live Nordpool.

    Frozen once a slot has ended (`end <= now`). Live replaces the in-progress
    slot and the forecast through the last available Nordpool slot. Stored
    futures that are not on the live curve are dropped unless live is empty
    (keep the last forecast). Slots older than ``keep_days`` are trimmed.
    """
    now_ts = float(clock.as_timestamp(now_dt))
    try:
        keep_days = float(keep_days)
    except (TypeError, ValueError):
        keep_days = float(SPOT_KEEP_DAYS)
    keep_after = now_ts - (keep_days * 86400.0)
    stored = normalize_spot_slots(stored)
    live = normalize_spot_slots(live)
    stored_by_start = {}
    for slot in stored:
        if slot[1] <= keep_after:
            continue
        stored_by_start[slot[0]] = slot
    merged = {}
...
def charger_mqtt_role(
    ...
    *,
    ...
    """What this charger should be doing: 22 kW, keep, leftover, or off."""
    if charger_full_power(
        policy,
        result,
        now_ts,
...
def charger_mqtt_command(
    ...
    *,
    ...
    if role == ROLE_KEEP:
        return ("on", int(keep_psm), int(group_lot), int(keep_amp))
