...
def main():
    ...
    def test_keep_until_unplug():
        ...
        assert_eq(
            ...
            ("on", 2, 50, 6),
            "keep command defaults to 3-phase 6 A with group lot 50",
        )
        assert_eq(
    ...
    def test_leftover_and_budget():
        ...
        assert_eq(lot3, 50, "16 kW keeps group lot at the fuse cap")
        assert_eq(psm3, 2, "3-phase")
        assert_eq(amp3, 23, "amp from leftover")
        lot5, psm5, amp5 = budget(5000, 6, 32, 50, 230, 32)
        assert_eq((lot5, psm5, amp5), (50, 1, 21), "5 kW first start is 1-phase 21 A, lot 50")
        ...
        assert_eq((lot8, psm8, amp8), (50, 2, 11), "8 kW is 3-phase 11 A, lot 50")
        lot12, psm12, amp12 = budget(12000, 6, 32, 50, 230, 32)
        assert_eq((lot12, psm12, amp12), (50, 2, 17), "12 kW is 3-phase 17 A, lot 50")
        lot_min3, psm_min3, amp_min3 = budget(4140, 6, 32, 50, 230, 32)
        assert_eq((lot_min3, psm_min3, amp_min3), (50, 1, 18), "4140 W first start is 1-phase 18 A, lot 50")
        ...
        assert_eq((lot1, psm1, amp1), (50, 1, 8), "2 kW is 1-phase 8 A, lot 50")
    ...
    def test_group_surplus_setpoint_keeps_app_priorities():
        ...
        assert_eq(leftover, (50, 1, 10), "pure surplus keeps group lot; amp is the leftover cap")
        assert_eq(
    ...
    def test_surplus_allocations_steal_second_charger_floor():
        ...
        assert_eq((lot, psm, amp), (50, 2, 17), "12 kW leftover amp 17 A, lot stays 50")
    ...
    def test_phase_hold_both_directions():
        ...
        assert_eq(amp_lo["psm"], 1, "5 kW stays 1-phase")
        assert_eq(amp_lo["wanted_psm"], 1, "1-phase can still offer 5 kW")
        assert_eq(amp_lo["arm_phase"], False, "no 1→3 hold while 1-phase can deliver")
        assert_eq(amp_lo["amp"], 21, "amp tracks leftover on 1-phase, not 7 A 3-phase")
        assert_eq(amp_hi["amp"], 30, "7 kW 1-phase is 30 A")
        assert_eq(amp_hi["wanted_psm"], 1, "7 kW still fits under 32 A 1-phase")
        stay6 = phase(6000, *args, last_psm=2)
        assert_eq((stay6["psm"], stay6["amp"], stay6["arm_phase"]), (2, 8, False), "6 kW keeps 3-phase")
