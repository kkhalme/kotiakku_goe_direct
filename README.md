...
| `sensor.kotiakku_goe_direct_window` | planned start | Planned window, including one that already ended. State is the start timestamp; `end`, avg, `windows`, `window_N_*`, and `raw` (configured Nordpool `raw_today` concatenated with `raw_tomorrow`) are attributes. `binary_sensor.kotiakku_goe_direct_window_active` is on while now is inside a window |
