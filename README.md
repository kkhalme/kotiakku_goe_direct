...
Windows are planned from `raw_today` / `raw_tomorrow` (or `today` / `tomorrow`). A valid window is a contiguous block whose duration is in [min, max] (default 2–5 h, step 0.25) and whose **every** 15-minute slot is at or below the price ceiling (default 0.1, native unit of the price sensor). Four independent greedy disjoint plans: cheapest, longest, earliest, and off-sun. Off-sun is the cheapest ranking after hours with ≥ 1 kWh expected solar are dropped; over-ceiling and surplus-hour gaps still split windows. After the greedy pick, under-ceiling holes between windows are filled (up to max hours) so a 15-minute minimum does not stop charging for a slightly dearer quarter. Off-sun also merges adjacent islands. A frozen window does not slide 15 minutes. Tomorrow’s curve typically appears sometime after 14:00 local. Off-sun also replans when remaining-today, tomorrow kWh, or the Off-sun hour knob change.
