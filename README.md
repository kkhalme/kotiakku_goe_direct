...
- After leftover MQTT, wait **15 s** before turning anyone off (a Tesla often needs several seconds to begin). Temporary over-draw is allowed. If high still does not take all leftover, the next charger already has it
...
| Offer wait | 15 s | After leftover MQTT, wait this long before `frc=1` on anyone. Over-draw is allowed. High stays on; leftover still goes to the next if high does not take all |
...
| Higher-priority car Idle / not taking, lower taking leftover | Both stay `frc=2` during the first **15 s** (temporary over-draw). After that, lower is first if high still is not taking. High stays `frc=2`. No 3 kW steal. Only when high starts taking is the lower reduced or dropped |
| Three chargers, unequal, middle Idle, high taking 10 kW of 12 kW | **9 kW + 3 kW** to high and the next in priority (Idle middle). Third waits |
