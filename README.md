...
Unknown SoC, solar, or house → treat as a blocked window. MQTT waits **2 s** after the first intended write (later sensor ticks in that window do not restart the timer). At flush, leftover and 22 kW are recomputed from **current** Kotiakku / Controller sensors. Instant charger `nrg` does not retune leftover `amp` unless take-started (100 W) or idle-Complete (400 W) crossed. The same command is sent only if live go-e `frc` / `amp` / `lot` / `psm` differ. The 15 min interval is the safety net.
...
Budget from leftover watts (floored amps, Finnish 230 V). Surplus `amp` follows Kotiakku solar/house/Controller (about every **5 min**), not instant charger `nrg`. Group `lot` stays at the fuse cap (default **50 A**). Keep the **active** phase while it can still offer leftover:
...
| MQTT apply | 2 s | Wait after the first intended write. Later events join that window. Flush recomputes from current sensors and sends only if live go-e state needs it |
