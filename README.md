...
| `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` | off | After charge complete keep: limited power until that car unplugs. HA turns it on after a finished pack (Complete after leftover surplus, cheap window, Force on, or 22 kW until-unplug; already Complete when plugged in; leftover starting on an already-finished car). Manual on/off. Unplug clears it |
...
| Leftover 1-phase, high-priority car plugs in and starts taking, lower still surplus | Drop the lower charger. Keep stays off even if go-e reports Complete a second or two later while force is still On (leftover stolen / group current starved, not a finished pack). Do not arm 3-phase keep on the lower car |
| Higher-priority car Complete / WaitCar, leftover 8 kW | Lower-priority charger gets leftover if it still meets 6 A. The Complete car gets **After charge complete keep** if it finished by itself. WaitCar still gets an offer so it can start; no steal |
...
| SolarPriority, cheap window ends, leftover still on | Leftover MQTT replaces 22 kW. No `frc=1` in between. If the car already Complete, after-charge-complete keep instead of leftover |
...
| Car Complete after HA was allowing charge (leftover surplus, cheap window, Force on, or 22 kW until-unplug) and keep enable is on | Keep switch on. Keep `psm` / `amp` (default 3-phase 6 A), `lot` 50, `frc` 2 until **its** car unplugs. Surplus skips it. A later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available |
| Plug in already Complete, or leftover starts on an already-finished car | Keep switch on (finished pack). Surplus skips it so leftover can go to other cars |
...
| HA stopped leftover or the cheap window while still WaitCar or Charging | Keep stays off (interrupt). Complete later is not a finished pack |
| Leftover moved to another taking charger, this car then Complete | Keep stays off (interrupt), including when Complete is 1–2 s later and force is still On. Same-tick Complete is not required |
