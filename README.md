...
Unknown SoC, solar, or house → treat as a blocked window. MQTT waits **2 s** after the first intended write (later sensor ticks in that window do not restart the timer). At flush, 22 kW is recomputed from current sensors. Surplus **start** uses live leftover. While surplus is on, hold/stop and leftover `amp` stay on the last **Kotiakku** sample (SoC, solar, or house) and move on the next report, including out of the 6 A floor. That report is consumed immediately, so a later Controller tick cannot reuse it. Controller and charger `nrg` do not move `amp`. Live leftover still updates `sensor.kotiakku_goe_direct_available_surplus` on those faster ticks. The 15 min interval republishes MQTT from the held leftover; it does not resample it. Instant charger `nrg` does not retune leftover `amp` unless take-started (100 W) or idle-Complete (400 W) crossed. The same command is sent only if live go-e `frc` / `amp` / `lot` / `psm` differ.
...
| Leftover amp cadence | Kotiakku report | While surplus is on, hold/stop and leftover `amp` move when Kotiakku SoC, solar, or house reports, including out of the 6 A floor. Controller and charger `nrg` do not resample. The 15 min interval republishes the hold; it does not resample. Live leftover stays on `sensor.kotiakku_goe_direct_available_surplus` |
