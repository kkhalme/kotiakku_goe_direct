...
Leftover MQTT is **SolarPriority** and **SolarAndGrid** only. **Force off** never charges (`frc=1`), including leftover. Surplus on/off follows leftover watts and SoC, not plug-in: Idle or unplugged surplus chargers still get leftover `lot` / `psm` / `amp` / `frc=2` so **Allowd to charge** / Force State can go on before WaitCar. A finished car (`Complete`) is skipped for leftover so other cars can take it. If that car **finished by itself** (Complete while HA was commanding it on — a cheap window, Force on, leftover WaitCar/Charging, or the 22 kW until-unplug switch) **and** `switch.kotiakku_goe_direct_after_charge_complete_keep_enable_<serial>` is on, HA turns on `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>`. That keeps charging allowed until unplug at `select.kotiakku_goe_direct_after_charge_complete_keep_phase` and `number.kotiakku_goe_direct_after_charge_complete_keep_a` (defaults **3-phase 6 A**, group `lot` 50, `frc=2`). That is for morning cabin precondition on house power, not for charging the pack. You can also turn that keep switch on or off by hand. It turns itself off when **that** car **unplugs**. A cheap window ending (or leftover stopping) while the car is still WaitCar or Charging does **not** turn it on. Car battery SoC is not available, so this is not a hard full-battery guarantee: a car that was not actually full may still trickle at keep amp. Surplus skips that serial while the keep switch is on. A later cheap window, Force on, or Force On Until Unplug still takes 22 kW. A single SolarPriority or SolarAndGrid charger always gets the leftover. When every listed surplus charger is surplus **and HA leftover priorities are equal**, the same leftover `lot` / `psm` / `amp` / `frc` is written to all of them (Idle included). That leftover `lot` is the group total. go-e load balancing splits it.
...
`switch.kotiakku_goe_direct_after_charge_complete_keep_enable_<serial>` (default **on**) is the per-charger gate for that auto-on. Off skips auto-on after a self-finish; you can still turn the keep switch on by hand. `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` is the same until-unplug lifetime at limited power. HA turns it on when the car **finishes by itself** (go-e Complete after HA commanded that charger on while the car was WaitCar or Charging — cheap window, Force on, leftover surplus, or 22 kW until-unplug). You can also turn it on or off yourself. MQTT uses `select.kotiakku_goe_direct_after_charge_complete_keep_phase` (default **3-phase** → `psm=2`) and `number.kotiakku_goe_direct_after_charge_complete_keep_a` (default **6 A**), with group `lot` 50 and `frc=2`. A cheap window ending (or leftover stopping) while the car is still WaitCar or Charging does **not** turn it on. Cabin precondition after a full charge can then take house power at keep amp. Surplus skips that serial; a later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available, so a not-actually-full car may trickle at keep amp until unplug.
...
| `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` | off | After charge complete keep: limited power until that car unplugs. HA turns it on after a self-finish (Complete after cheap window, Force on, leftover, or 22 kW until-unplug). Manual on/off. Unplug clears it |
...
| Car Complete while HA was commanding on (window, Force on, leftover WaitCar/Charging, or 22 kW until-unplug) and keep enable is on | `after_charge_complete_keep` switch on. Keep `psm` / `amp` (default 3-phase 6 A), `lot` 50, `frc` 2 until **its** car unplugs. Surplus skips it. A later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available |
| Car Complete as above, keep enable off | Keep switch stays off unless turned on by hand |
| After charge complete keep switch turned on by hand | Same keep MQTT until **its** car unplugs, even if the car never reported Complete or keep enable is off |
| Car still WaitCar or Charging when the cheap window (or leftover) stops | Keep switch stays off. Leftover or `frc` 1 as today |
