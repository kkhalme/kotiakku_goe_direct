...
Full-power MQTT on that charger: `fup` false, `psm=2`, `amp=32`, `lot=50`, `frc=2`. After-charge-complete keep MQTT: `fup` false, keep `psm` / `amp`, `lot=50`, `frc=2`. One decision writes MQTT: 22 kW, keep, leftover, or `frc=1`. A cheap hour ending does **not** force-off a charger leftover is about to write — leftover MQTT replaces 22 kW. If the keep switch is on, keep MQTT replaces 22 kW instead of leftover or `frc=1`. If leftover is not writing and keep is off, stop is `frc=1` then `fup` false. Every trigger only schedules; a **2 s** window from the first schedule recomputes desired from current sensors and publishes at most one command per serial, and only if live `frc` / `amp` / `lot` / `psm` need it. Window start/end force that publish so a WaitCar that still has **Allowd to charge** off / **0 A** is retried even if HA already sent 22 kW once. Live `frc=1` or `amp` 0 after an On write is a failed start, not a reason to wait. The 15 min interval is the same apply (safety net).
