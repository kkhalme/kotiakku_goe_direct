...
`switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` is the same until-unplug lifetime at limited power. HA turns it on when the car **finishes by itself** (go-e Complete while HA was commanding that charger on). You can also turn it on or off yourself. MQTT uses `select.kotiakku_goe_direct_after_charge_complete_keep_phase` (default **3-phase** → `psm=2`) and `number.kotiakku_goe_direct_after_charge_complete_keep_a` (default **6 A**), with group `lot` 50 and `frc=2`. A cheap window ending (or leftover stopping) while the car is still Charging does **not** turn it on. Cabin precondition after a full charge can then take house power at keep amp. Surplus skips that serial; a later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available, so a not-actually-full car may trickle at keep amp until unplug.
...
| `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` | off | After charge complete keep: limited power until that car unplugs. HA turns it on after a self-finish (Complete). Manual on/off. Unplug clears it |
...
| Car Complete while HA was commanding on (window, Force on, leftover, or 22 kW until-unplug) | `after_charge_complete_keep` switch on. Keep `psm` / `amp` (default 3-phase 6 A), `lot` 50, `frc` 2 until **its** car unplugs. Surplus skips it. A later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available |
| After charge complete keep switch turned on by hand | Same keep MQTT until **its** car unplugs, even if the car never reported Complete |
| Car still Charging when the cheap window (or leftover) stops | Keep switch stays off. Leftover or `frc` 1 as today |
