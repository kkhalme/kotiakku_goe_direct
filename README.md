...
Leftover MQTT is **SolarPriority** and **SolarAndGrid** only. **Force off** never charges (`frc=1`), including leftover. Surplus on/off follows leftover watts and SoC, not plug-in: Idle or unplugged surplus chargers still get leftover `lot` / `psm` / `amp` / `frc=2` so **Allowd to charge** / Force State can go on before WaitCar. A finished car (`Complete`) is skipped for leftover so other cars can take it. Keep auto-on is for a **finished pack**: go-e Complete after HA was allowing that charger to charge (leftover surplus WaitCar/Charging, a cheap window, Force on, or 22 kW until-unplug) **and** `switch.kotiakku_goe_direct_after_charge_complete_keep_enable_<serial>` is on. That includes surplus leftover — leftover Complete is a finished charge, not a leftover skip with no keep. HA then turns on `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>`. That keeps charging allowed until unplug at `select.kotiakku_goe_direct_after_charge_complete_keep_phase` and `number.kotiakku_goe_direct_after_charge_complete_keep_a` (defaults **3-phase 6 A**, group `lot` 50, `frc=2`). That is for morning cabin precondition on house power, not for charging the pack. You can also turn that keep switch on or off by hand. It turns itself off when **that** car **unplugs**. **Force off** never allows a charge, so it does not auto-on keep. Plugging in a car that is already Complete is not a finished charge on this session. A cheap window or leftover surplus that **stops while the car is still WaitCar or Charging** is an interrupt — Complete later does not auto-on keep. Car battery SoC is not available, so this is not a hard full-battery guarantee: a car that was not actually full may still trickle at keep amp. Surplus skips that serial while the keep switch is on. A later cheap window, Force on, or Force On Until Unplug still takes 22 kW. A single SolarPriority or SolarAndGrid charger always gets the leftover. When every listed surplus charger is surplus **and HA leftover priorities are equal**, the same leftover `lot` / `psm` / `amp` / `frc` is written to all of them (Idle included). That leftover `lot` is the group total. go-e load balancing splits it.
...
| `switch.kotiakku_goe_direct_after_charge_complete_keep_<serial>` | off | After charge complete keep: limited power until that car unplugs. HA turns it on after a finished pack (Complete after leftover surplus, cheap window, Force on, or 22 kW until-unplug). Manual on/off. Unplug clears it |
...
| Plug in already Complete, or leftover starts on an already-finished car | Keep stays off — no charge session finished on this cable |
