...
Unknown SoC, solar, or house → treat as a blocked window. MQTT waits **2 s** after the first intended write (later sensor ticks in that window do not restart the timer). At flush, 22 kW is recomputed from current Kotiakku / Controller sensors. Surplus start/hold/stop and leftover `amp` use leftover watts **held for 5 min** after a Kotiakku/Controller sample (or the 15-min safety apply). Live leftover still updates `sensor.kotiakku_goe_direct_available_surplus` every tick — house including the car while the Controller 5-min mean lags must not bounce `amp` (30 A ↔ 6 A). Instant charger `nrg` does not retune leftover `amp` unless take-started (100 W) or idle-Complete (400 W) crossed. The same command is sent only if live go-e `frc` / `amp` / `lot` / `psm` differ. The 15 min interval is the safety net.
...
Budget from leftover watts (floored amps, Finnish 230 V). Surplus `amp` and start/hold/stop use leftover watts sampled on the Kotiakku solar/house/Controller cadence (typically **5 min**), not every house tick and not instant charger `nrg`. House can include the car while the Controller 5-min mean lags: live leftover then collapses and would bounce `amp` (30 A ↔ 6 A) every few seconds. The held leftover avoids that; Tesla can ramp. Up to 5 min of grid over-draw is accepted. `sensor.kotiakku_goe_direct_available_surplus` stays live. Group `lot` stays at the fuse cap (default **50 A**). Keep the **active** phase while it can still offer leftover:
...
| Leftover amp cadence | 5 min | Surplus start/hold/stop and leftover `amp` sample leftover watts this often (new Kotiakku/Controller reading, or the 15-min safety apply). House ticks faster than that must not bounce `amp`. Live leftover stays on `sensor.kotiakku_goe_direct_available_surplus` |
