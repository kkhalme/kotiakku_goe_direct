...
Full-power MQTT on that charger: `fup` false, `psm=2`, `amp=32`, `lot=50`, `frc=2`. After-charge-complete keep MQTT: `fup` false, keep `psm` / `amp`, `lot=50`, `frc=2`. One decision writes MQTT: 22 kW, keep, leftover, or `frc=1`. A cheap hour ending does **not** force-off a charger leftover is about to write — leftover MQTT replaces 22 kW. If the keep switch is on, keep MQTT replaces 22 kW instead of leftover or `frc=1`. If leftover is not writing and keep is off, stop is `frc=1` then `fup` false. Every trigger only schedules; a **2 s** window from the first schedule recomputes desired from current sensors and publishes at most one command per serial, and only if live `frc` / `amp` / `lot` / `psm` need it. The 15 min interval is the same apply (safety net).
...
| `sensor.kotiakku_goe_direct_spot` | current slot | Realized Nordpool past (kept ~8 days) plus live `raw_today`/`raw_tomorrow` through the last forecast slot. Attribute `raw` is `[{start, end, value}, …]` for charts; `source` and `forecast_end` say where it came from |
...
| Higher-priority car Complete / WaitCar, leftover 8 kW | Idle Complete is not leftover-offered. Lower-priority charger gets leftover if it still meets 6 A. The Complete car gets **After charge complete keep** after 60 s if it is not a steal victim. WaitCar still gets an offer so it can start; no steal |
...
| HA stopped leftover or the cheap window while still WaitCar or Charging | Keep stays off (interrupt / `KEEP_CUT`). Complete later is not a finished pack |
| Leftover writing on another taking higher-priority charger, this worse-priority car idle Complete | Keep stays off (steal victim), including when Complete is 1–2 s later and force is still On. Not a `KEEP_CUT` unless leftover already stopped while WaitCar/Charging |
...
Live Home Assistant: one dashboard tab. Install [statistics-graph-chart-card](https://github.com/cataseven/Statistics-Graph-Chart-Card) and [apexcharts-card](https://github.com/RomRider/apexcharts-card) from HACS, then copy [`homeassistant/dashboards/kotiakku_goe_direct_48h.yaml`](homeassistant/dashboards/kotiakku_goe_direct_48h.yaml) as a YAML dashboard, or paste the `views:` list into a UI dashboard's raw editor. Spot-only card: [`homeassistant/dashboards/kotiakku_goe_direct_spot.yaml`](homeassistant/dashboards/kotiakku_goe_direct_spot.yaml).

The spot chart is `custom:statistics-graph-chart-card` on `sensor.kotiakku_goe_direct_spot` (`data_attribute: raw`). That helper freezes realized prices (and can backfill older days from the Nordpool entity's recorder history) and always appends the live `raw_today`/`raw_tomorrow` curve through the last available slot — so a week-long chart (`hours_to_show: 168`, omit `graph_start` / `graph_end_hour`) shows the past week and still extends into the forecast. The bundled 48 h card keeps `graph_start: day` and `graph_end_hour: now+48` so the axis stays through tomorrow even before day-ahead prices exist. Cheap windows are transparent span annotations from `sensor.kotiakku_goe_direct_window` (`window_1_*` / `window_2_*`) plus a thin state-strip under the plot (`binary_sensor.kotiakku_goe_direct_window_active`). Each charger's 22 kW blocks are the `*_full_power` strips stacked above that window strip. Replace fake serials `111111` / `222222`. One charger: delete the charger 2 strip. Leftover past is `|solar| − |house| + |ev|` from recorder 5-minute statistics of the three power sensors you already have — edit those entity ids (and kW vs W) in the leftover `data_generator`. Display-only; it does not write MQTT.
