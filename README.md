...
- While leftover is on a lower-priority charger (steal or next-as-first), higher-priority surplus chargers stay `frc=2`. Do not force them off or they cannot start taking again
...
| Offer wait | 15 s | After leftover MQTT, wait this long for the car to start taking before leftover goes to the next charger |
...
| Higher-priority car Idle / not taking, lower taking leftover | First **15 s** leftover stays on high (`frc=2`). Then lower gets leftover as first. High still gets leftover MQTT (`frc=2`) so it can start taking again. No 3 kW steal |
