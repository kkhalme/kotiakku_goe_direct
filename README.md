...
Leftover MQTT is **SolarPriority** and **SolarAndGrid** only. **Force off** never charges (`frc=1`), including leftover. Surplus on/off follows leftover watts and SoC, not plug-in: Idle or unplugged surplus chargers still get leftover `lot` / `psm` / `amp` / `frc=2` so **Allowd to charge** / Force State can go on before WaitCar. A finished car (`Complete`) is skipped for leftover so other cars can take it. If that car **finished by itself** (Complete while HA was commanding it on — a cheap window, Force on, or leftover — not because the window or leftover cut the session), HA then keeps **3-phase 6 A until unplug** (`psm=2`, `amp=6`, group `lot` 50, `frc=2`). That is for morning cabin precondition on house power, not for charging the pack. Car battery SoC is not available, so this is not a hard full-battery guarantee: a car that was not actually full may still trickle at 6 A. It clears when **that** car **unplugs**, or if the policy is Force off. A later cheap window, Force on, or Force On Until Unplug still takes 22 kW. Surplus skips that serial while the 6 A keep is on (same as full-power: do not shrink group `lot`). A single SolarPriority or SolarAndGrid charger always gets the leftover. When every listed surplus charger is surplus **and HA leftover priorities are equal**, the same leftover `lot` / `psm` / `amp` / `frc` is written to all of them (Idle included). That leftover `lot` is the group total. go-e load balancing splits it.
...
After a car **finishes charging by itself** (go-e Complete while HA was commanding it on), HA also keeps that charger on until unplug, but at **3-phase 6 A** rather than 22 kW. That is automatic — it does not flip the 22 kW switch. A cheap window ending (or leftover stopping) while the car is still Charging does **not** arm this: that session was cut. Cabin precondition after a full charge can then take house power at ~4.1 kW. Surplus skips that serial; a later 22 kW window / Force on / Force On Until Unplug still wins. Force off cancels it. Car SoC is not available, so a not-actually-full car may trickle at 6 A until unplug.

Leftover surplus writes **SolarPriority** and **SolarAndGrid** chargers. Surplus skips **Force off**, and also skips a charger that is already full-power (**Force on**, a cheap window, or Force On Until Unplug) or in the post-Complete **3-phase 6 A until unplug** keep. Spot-price windows and force-on do **not** read Kotiakku SoC / solar / house. Gridle going unknown only affects leftover surplus. HA still does not write app charger priorities (`lop`).
...
Full-power MQTT on that charger: `fup` false, `psm=2`, `amp=32`, `lot=50`, `frc=2`. Post-Complete keep MQTT: `fup` false, `psm=2`, `amp=6`, `lot=50`, `frc=2`. One decision writes MQTT: 22 kW, 6 A keep, leftover, or `frc=1`. A cheap hour ending does **not** force-off a charger leftover is about to write — leftover MQTT replaces 22 kW. If the car already finished by itself, 6 A keep replaces 22 kW instead of leftover or `frc=1`. If leftover is not writing and keep is not armed, stop is `frc=1` then `fup` false. Every trigger only schedules; a **2 s** window from the first schedule recomputes desired from current sensors and publishes at most one command per serial, and only if live `frc` / `amp` / `lot` / `psm` need it. The 15 min interval is the same apply (safety net).
...
| `switch.kotiakku_goe_direct_until_unplug_<serial>` | off | Force On Until Unplug: 22 kW until that car unplugs. Stays on at a full battery (Complete). Does not change the policy select. Separate from the automatic 3-phase 6 A keep after a self-finish |
...
| Higher-priority car Complete / WaitCar, leftover 8 kW | Lower-priority charger gets leftover if it still meets 6 A. The Complete car is kept at 3-phase 6 A until unplug if it finished by itself. WaitCar still gets an offer so it can start; no steal |
...
| SolarPriority, cheap window ends, leftover still on | Leftover MQTT replaces 22 kW. No `frc=1` in between. If the car already Complete, 3-phase 6 A keep instead of leftover |
| SolarPriority, cheap window ends while still Charging | Leftover or `frc=1`. Not 6 A keep — the window cut the session |
...
| Car Complete while HA was commanding on (window, Force on, or leftover) | That charger **3-phase 6 A** (`psm` 2, `amp` 6, `lot` 50, `frc` 2) until **its** car unplugs. Surplus skips it. A later 22 kW window / Force on / Force On Until Unplug still wins. Car SoC is not available |
| Car still Charging when the cheap window (or leftover) stops | No 6 A keep. Leftover or `frc` 1 as today |
